...
- **Week-6 step-4 result (`reports/week6_baselines_v1/`, 40k LHS, 32k train / 3.9k val / 4.0k test).** 16/18 acceptance rows pass on the test split. The two failures are both **Ridge** (R² 0.77 on range, 0.66 on energy margin) — exactly the diagnostic we want: the linear baseline cannot model the multiplicative coupling of solar geometry × duty cycle × mass, and gets crushed on `polar_prospecting` energy margin (R² ≈ −59) where the non-linear models all hold R² ≥ 0.79. The **joint MLP wins every primary target** by a small margin over per-target XGBoost: range R² 0.9985 (XGB 0.9981), energy-margin raw R² 0.9958 (XGB 0.9869), slope R² 0.9978 (XGB 0.9857), mass R² 0.9997 (XGB 0.9991). XGBoost wins feasibility classification (AUC 0.998 vs LogReg 0.988). Total fit wall-clock 30.6 s on 11 cores. See `project_log.md` 2026-04-25 entry for full numbers and discussion.
